...
const LiClass = props => css`
  ...
  color: ${props.state === 'todo' ? 'black': '#b83b5e'};
  border-bottom: 5px solid ${props.state === 'todo' ? '#ef885d' : '#b83b5e'};
  &:before {
    position: relative;
    bottom: -3.99rem;
    float: left;
    ...
    background-color:${props.state === 'todo' ? '#b83b5e' : '#b83b5e'};
    width: 1.2em;
    line-height: ${props.state === 'todo' ? '1.2em' : '1.4em'};
    border-radius: ${props.state === 'todo' ? '0' : '50%'};
    border: ${props.state === 'todo' ? 'white' : 'solid 10px rgba(184, 59, 94, 0.56);'};
  }
  &:hover,
  &::before {
    color: #0FA0CE;
...
export default function MultiStep(props) {
  ...
  const renderNav = (show) =>
    ...
    const {component} = props.steps[compState]
  return (
    <div onKeyDown={handleKeyDown}>
      <Ol>{renderSteps()}</Ol>
      <div>{component({next})}</div>
